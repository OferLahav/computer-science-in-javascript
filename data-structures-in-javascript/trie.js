(function(scope){
	...
	Trie.prototype.getNextTier = function(word){
	  if(!this.root) {
		    return false;
		  }
	  return this._getNextTier(this.root, word);
	};

	Trie.prototype._getNextTier = function(node, word){
	  if(!node) {
	    return false;
	  }
	  var letter = word.charAt(0);
	  ...
	  if(child) {
	    ...
	    if(!remainder && child.isWord) {
	      ...
	      return this._getNextTier(child, remainder);
	    }
	  } else {
